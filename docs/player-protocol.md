...
Player-config v1 optionally includes organization-owned `playback.regionalFormat`: a canonical BCP-47 locale, IANA timezone, date-format preference, time-format preference, and resolved first day of week. It is added within the extensible `playback` object while the v1 top level stays closed. Existing v1 Players can ignore the unknown playback property and continue using the rest of the configuration. A new Player talking to an older Server sees no profile and uses the legacy fallback described by that Player's formatting contract; it must not substitute the device locale. Settings changes still advance config and manifest revisions through the existing invalidation path.
