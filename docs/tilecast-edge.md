...
- runs with no capabilities, `NoNewPrivileges=yes`, `RestrictAddressFamilies=AF_UNIX`, `PrivateDevices=yes`, `ProtectSystem=strict`, `ProtectHome=tmpfs` with only its runtime directory visible, and `MemoryDenyWriteExecute=yes`. It does not set `CapabilityBoundingSet=`: a user manager cannot drop the bounding set, and the unit would fail to start with `218/CAPABILITIES`.
