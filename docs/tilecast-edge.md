...
- runs with no capabilities and two sandbox layers. The seccomp layer (`NoNewPrivileges=yes`, `RestrictAddressFamilies=AF_UNIX`, `SystemCallFilter=@system-service`, `MemoryDenyWriteExecute=yes`, `RestrictNamespaces=yes`) holds on every host. The mount layer (`PrivateUsers=yes`, `ProtectSystem=strict`, `ProtectHome=tmpfs` with only its runtime directory visible, and `InaccessiblePaths=` for the daemon's state, `/run/tilecast`, `/dev/snd`, `/dev/dri`, `/dev/input` and `/dev/bus/usb`) needs a user namespace for the unit; where a host restricts unprivileged user namespaces (the Ubuntu 24.04 AppArmor default), systemd skips it and the bridge still starts. The unit sets nothing that shrinks the capability bounding set (`CapabilityBoundingSet=`, `PrivateDevices=`, `ProtectKernelModules=`, `ProtectKernelLogs=`, `ProtectClock=`): a user manager cannot do that, and every start would fail with `218/CAPABILITIES`.
