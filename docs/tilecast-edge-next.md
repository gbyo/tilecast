...
- run commands or apply player configuration (M4);
- pair on its own; a fresh installation without legacy state cannot bind (M5);
- report proof of play or telemetry (M8).

Do not migrate a production screen before milestones M4, M5 and M7 are done.
...
| M4 to M6, M7 to M10, M11 and M12 readiness             | One pull request per group, in that order. M4 starts from `server_link` (the socket already wakes on `config.changed` and `commands.available`) and the legacy import's command-idempotency rows. |
