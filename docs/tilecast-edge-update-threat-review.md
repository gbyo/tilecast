...
- It opens the object with `openat2` and `RESOLVE_NO_SYMLINKS | RESOLVE_NO_MAGICLINKS`, `O_NOFOLLOW`, `O_NONBLOCK` and `O_NOCTTY`, so no component of the path can be a link and a FIFO cannot block it.
...
| `NoNewPrivileges=yes`, `RestrictSUIDSGID=yes`                                                                                               | No gain of privilege through an executed file.                                                                                        |
...
- manual recovery: install the candidate again through a new deployment after its fault is fixed, or restore a copy of the database. The helper keeps the database as it is for this.
