...
The U.S.-specific automated NWS integration is configured in Studio under Plugins → US Weather Alerts (stable plugin ID `emergency_alerts`). The server needs outbound HTTPS access to `api.weather.gov` for the state/county/forecast-zone picker and active alerts. Monitoring is disabled by default, identifies the installation in its User-Agent, polls no more frequently than once per minute, records upstream health without response bodies or secrets, and is not a replacement for local life-safety systems. CAP Alerts Data Sources are a separate feed-driven display option and do not activate automated Takeovers.
