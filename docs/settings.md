...
The organization settings `organization.locale`, `organization.timezone`, `organization.date_format`, `organization.time_format`, and `organization.first_day_of_week` describe the installation's regional behavior for signage and organization-owned data. The locale is a validated, canonical BCP-47 tag, not a list of shipped Studio translation languages. The timezone is an IANA identifier. `first_day_of_week=locale` derives its value from CLDR week data; explicit weekday choices remain available. The default remains Sunday so existing installations retain their effective week boundary until an administrator changes it.
