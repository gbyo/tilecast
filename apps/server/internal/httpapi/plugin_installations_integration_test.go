...
func TestPluginInstallationAPI(t *testing.T) {
	withActivityDatabase(t, func(env activityTestEnvironment) {
		...
		if len(items) != 5 {
			t.Fatalf("catalog items = %d, want 5", len(items))
		}

		for _, role := range []string{"viewer", "editor"} {
			if status, _ = call(role, http.MethodPost, "/api/v1/plugins/noise_meter/install", true, ""); status != http.StatusForbidden {
				t.Fatalf("%s install status = %d, want 403", role, status)
			}
			if status, _ = call(role, http.MethodDelete, "/api/v1/plugins/noise_meter/installation", true, ""); status != http.StatusForbidden {
				t.Fatalf("%s remove status = %d, want 403", role, status)
			}
		}
		if status, body = call("owner", http.MethodPost, "/api/v1/plugins/noise_meter/install", false, ""); status != http.StatusForbidden || errorCode(body) != "csrf_failed" {
			t.Fatalf("install without CSRF = %d %v", status, body)
		}

		// Configuring before installing is refused rather than installing implicitly.
		meter := `{"name":"Cafeteria","message":"","warningLevel":60,"loudLevel":80,"sensitivity":100,"triggerHoldMs":1000,
			"clearHoldMs":3000,"displayMode":"overlay","heightPx":96,"enabled":true,"historyEnabled":false,"historyRetentionDays":7,
			"historyActiveHoursOnly":false,"scheduleEnabled":false,"scheduleDaysOfWeek":[],"scheduleTimezone":"UTC","targetScope":"all","targetIds":[]}`
		if status, body = call("owner", http.MethodPost, "/api/v1/plugins/noise-meter/instances", true, meter); status != http.StatusConflict || errorCode(body) != "plugin_not_installed" {
			t.Fatalf("configure uninstalled plugin = %d %v", status, body)
		}

		if status, body = call("administrator", http.MethodPost, "/api/v1/plugins/noise_meter/install", true, ""); status != http.StatusCreated {
		...
		if status, _ = call("owner", http.MethodPost, "/api/v1/plugins/noise_meter/install", true, ""); status != http.StatusOK {
		...
		if status, body = call("owner", http.MethodPost, "/api/v1/plugins/noise-meter/instances", true, meter); status != http.StatusCreated {
			t.Fatalf("create meter after install = %d %v", status, body)
		}
		status, body = call("owner", http.MethodDelete, "/api/v1/plugins/noise_meter/installation", true, "")
		...
		if details["pluginId"] != "noise_meter" || len(resources) != 1 || resources[0].(map[string]any)["kind"] != "noise_meter_instance" {
			t.Fatalf("plugin_in_use details = %v", details)
		}

		if status, _ = call("owner", http.MethodDelete, "/api/v1/plugins/brand_bug/installation", true, ""); status != http.StatusNoContent {
			t.Fatalf("remove never-installed plugin = %d, want idempotent 204", status)
		}
		if status, _ = call("owner", http.MethodPost, "/api/v1/plugins/brand_bug/install", true, ""); status != http.StatusCreated {
			t.Fatal("install brand bug")
		}
		if status, _ = call("administrator", http.MethodDelete, "/api/v1/plugins/brand_bug/installation", true, ""); status != http.StatusNoContent {
			t.Fatalf("remove empty plugin = %d", status)
		}
		var removed int
		if err := env.pool.QueryRow(ctx, `SELECT count(*) FROM audit_logs WHERE action='plugin.removed' AND resource_id='brand_bug'`).Scan(&removed); err != nil || removed != 1 {
