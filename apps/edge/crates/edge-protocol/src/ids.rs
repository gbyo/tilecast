...
//! representation, so stored and compared values never disagree on spelling.
//!
//! The newtypes are distinct types so an installation ID can never be passed
//! where a player ID is expected.
...
uuid_id!(
    ...
    /// (`organization_settings.installation_id`). A stored device credential
    /// is only ever sent to the installation it was issued by.
    InstallationId
);
uuid_id!(
    /// The player-generated installation UUID (`playerInstallationId` in
    /// pairing metadata). Generated once on the device and preserved across
    /// the legacy import, so the server already knows it. It identifies the
    /// device; the device credential authenticates it.
    PlayerId
);
uuid_id!(
    /// A logical Tilecast screen record on the server. A device's screen can
    /// change through re-pairing or hardware replacement; its player ID cannot.
    ScreenId
);
uuid_id!(
...
mod tests {
    ...
    fn parses_only_canonical_form() {
        let parsed: PlayerId = ID.parse().expect("canonical");
        ...
        for bad in [
            ...
            assert!(bad.parse::<PlayerId>().is_err(), "{bad} must be rejected");
        }
    }
