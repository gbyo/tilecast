//! The Tilecast Edge capability model (docs/tilecast-edge.md §12).
//!
//! A capability describes one thing a device can or cannot do, with enough
...
//! ```
//!
//! State meanings are exact, so a later server or Studio view can show them
//! unchanged:
//!
//! | state | meaning |
//! | --- | --- |
...
//! materially ([`Capability::materially_equal`]). The daemon keeps only the
//! current snapshot, never a per-refresh history.
...
pub const CATEGORIES: &[&str] =
    &["renderer", "video", "time", "display", "audio", "input", "network", "system", "external_presentation"];
...
/// The complete current capability set of a device.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
...
mod tests {
    ...
    fn material_equality_ignores_observation_time() {
        let id = CapabilityId::new("audio.pipewire").expect("id");
    ...
    fn snapshot_sorts_and_rejects_duplicates() {
        ...
        let snapshot =
            CapabilitySnapshot::new(3, at(), vec![make("time.host_sync"), make("audio.pipewire")]).expect("ok");
        assert_eq!(snapshot.capabilities[0].id.as_str(), "audio.pipewire");
        assert_eq!(
            CapabilitySnapshot::new(3, at(), vec![make("time.host_sync"), make("time.host_sync")]).unwrap_err(),
            SnapshotError::Duplicate("time.host_sync".into())
        );
        let json = serde_json::to_string(&snapshot).expect("json");
        let duplicated = json.replace("audio.pipewire", "time.host_sync");
