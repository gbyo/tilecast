...
//! a timeout each, and assembles the device's complete capability list. A
...
//! change) is `edge_state::repo::capabilities`; `tilecastctl capabilities`
//! reads it over IPC. Providers only observe.
//!
...
//!
//! Never branch on an OS or platform name elsewhere to decide what a device
...
mod tests {
    ...
    async fn providers_are_isolated_and_failures_degrade() {
        ...
        registry.register(Arc::new(Fixed("cec", "display.cec.power", CapabilityState::Available)));
        ...
        assert_eq!(
            ...
            vec!["audio.pipewire", "display.cec.power"]
        );
        ...
        let cec = second.iter().find(|c| c.id.as_str() == "display.cec.power").expect("unaffected");
        assert_eq!(cec.state, CapabilityState::Available);
    }
}
