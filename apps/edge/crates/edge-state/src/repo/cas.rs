...
type RawObject = (String, i64, String, Option<String>, String, String, i64, i64, i64);

fn object_from_raw(raw: RawObject) -> Result<ObjectRecord> {
    let (sha, size, domain, content_type, source, verify, verified, created, accessed) = raw;
    Ok(ObjectRecord {
...
const OBJECT_COLUMNS: &str = "sha256, size_bytes, domain, content_type, source_kind, verify_state, verified_at_ms, created_at_ms, last_accessed_at_ms";
...
/// refreshes verification.
pub fn put_object(connection: &Connection, record: &ObjectRecord) -> Result<()> {
    connection.execute(
        &format!(
            "INSERT INTO cas_objects ({OBJECT_COLUMNS}) VALUES (?1, ?2, ?3, ?4, ?5, 'verified', ?6, ?7, ?8)
...
/// Releases every pin of `reason` whose holder starts with `prefix` except
/// the holders in `keep`. The caller supplies a constant prefix; holders are
/// daemon-built identifiers, never user values.
pub fn retain_holders(connection: &Connection, reason: PinReason, prefix: &str, keep: &[String]) -> Result<usize> {
    let pattern = format!("{prefix}%");
    let mut statement =
        connection.prepare("SELECT DISTINCT holder FROM cas_pins WHERE reason = ?1 AND holder LIKE ?2")?;
    let holders: Vec<String> =
        statement.query_map(params![reason.as_str(), pattern], |row| row.get(0))?.collect::<rusqlite::Result<_>>()?;
    let mut released = 0;
    for holder in holders.iter().filter(|holder| !keep.contains(holder)) {
        released += connection
            .execute("DELETE FROM cas_pins WHERE reason = ?1 AND holder = ?2", params![reason.as_str(), holder])?;
    }
    Ok(released)
}
