...
use edge_cas::{
    BlobSource, CasError, CasEvent, ContentStore, FetchError, FetchObserver, FetchRequest, Fetcher, IngestMeta,
...
#[tokio::test]
async fn fetch_verifies_promotes_and_announces() {
    let env = env();
    ...
    assert_eq!(
        ...
        CasEvent::Added { digest: record.sha256, size_bytes: DATA.len() as u64, peerable: true }
    );
...
async fn corrupt_same_size_bytes_never_enter_the_store() {
    ...
    let sources: Vec<Arc<dyn BlobSource>> =
        vec![Arc::new(Scripted::new("evil-peer", &evil)), Arc::new(Scripted::new("origin", DATA))];
...
async fn interrupted_transfer_resumes_from_another_source() {
    ...
    let store = store(&env).await;
    let fetcher = Fetcher::new(store.clone(), 1);
    let mut first = Scripted::new("peer-a", DATA);
...
async fn reconciliation_handles_every_crash_boundary() {
    ...
    assert!(!adopted_record.peerable, "adopted objects are not peerable until a manifest says so");
