...
pub struct IngestMeta {
    ...
    /// Whether peers may fetch this object (RFC §14.2). Only objects that
    /// are already eligible for this authenticated player may be peerable.
    pub peerable: bool,
    pub source: RecordSource,
}

/// Best-effort store notifications for mesh availability hints.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CasEvent {
    Added { digest: Sha256Digest, size_bytes: u64, peerable: bool },
    Removed { digest: Sha256Digest },
}
...
struct Inner {
    ...
    events: broadcast::Sender<CasEvent>,
}
...
impl ContentStore {
    ...
    pub fn subscribe(&self) -> broadcast::Receiver<CasEvent> {
    ...
    async fn remove_unchecked(&self, digest: &Sha256Digest) -> Result<(), CasError> {
        ...
        let _ = self.inner.events.send(CasEvent::Removed { digest: *digest });
        Ok(())
    }
    ...
    pub async fn begin_write(
        ...
        if let Some(record) = self.ensure_verified(&digest).await? {
            if meta.peerable && !record.peerable {
                let upgraded = ObjectRecord { peerable: true, ..record };
                self.inner.db.run(move |c| repo::put_object(c, &upgraded)).await?;
            }
            return Ok(None);
        }
...
impl WriteSession {
    ...
    pub async fn commit(mut self) -> Result<ObjectRecord, CasError> {
        ...
        let _ = self.store.inner.events.send(CasEvent::Added {
            digest: self.digest,
            size_bytes: self.expected_size,
            peerable: self.meta.peerable,
        });
