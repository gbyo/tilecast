...
//! (`edge-server`) and local files (legacy import, fixtures). Any later
//! source implements the same trait; the store's verification does not
//! change for it.
//!
...
pub trait BlobSource: Send + Sync + std::fmt::Debug {
    ...
    /// A short identifier for logs: `origin`, `local`…
