//! Tilecast Edge content-addressed store (docs/tilecast-edge.md §9).
//!
//! # Identity and layout
//!
...
//! ```
//!
//! No filename, URL, MIME type or server input ever becomes part of a path.
//!
...
//! # Extension points
//!
//! * [`source::BlobSource`] — where bytes come from. Edge 1 has the Tilecast
//!   Server origin and local files (legacy import, fixtures); a local verified
//!   object never needs a source at all. [`fetch::Fetcher`] tries sources in
//!   the order given and handles resume, restart-on-200, integrity failure
//!   and fallback, so a later source (a release artifact store, or a peer if
//!   measurements ever justify one) plugs in without touching verification.
//! * [`policy::EvictionPolicy`] — which unpinned objects to evict.
//! * [`fetch::FetchObserver`] — per-attempt outcomes for byte counters and
//!   diagnostics.
...
pub use store::{CasError, ContentStore, IngestMeta, StorePolicy, WriteSession};
