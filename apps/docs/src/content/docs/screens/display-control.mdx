...
The built-in Linux providers use HDMI-CEC for power and input, and DDC/CI for brightness and volume. The Linux host needs `cec-ctl` for HDMI-CEC or `ddcutil` for DDC/CI, access to the connected device, and a compatible display or adapter. Android Players do not provide Display Control.
...
The group page offers Mute actions, but the current built-in Linux providers do not report mute capability. A screen without a reported capability is listed as unsupported.
