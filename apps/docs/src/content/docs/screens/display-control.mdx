...
The Tilecast Edge M9 preview is replacing those subprocess providers with direct, bounded Linux CEC and I²C/DDC device access. Edge can also probe DDC/CI mute when a display exposes that feature. This is still preview behavior and is not a reason to remove `cec-ctl` or `ddcutil` from a stable Linux Player installation.
