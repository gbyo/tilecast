...
Set **Wall mode** to **Span** when the screens form one logical canvas. See [Build a Span video wall](../span-video-walls/) for panel setup, Player requirements, and video preparation.
