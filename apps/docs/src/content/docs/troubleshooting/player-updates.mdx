...
If a screen shows **Failed**, it kept playing its current release. Common reasons:
...
If the screen shows its recovery screen after a rollback, the new release changed the local state database to a version the previous release can't read. See [Tilecast Edge troubleshooting](../../edge/troubleshooting/#recovery-screen-after-an-update).
