...
- tilecast/
  - apps/ Product applications.
    ...
    - edge/ Tilecast Edge, the Linux player in development: a Rust daemon, a WPE WebKit renderer, and a small user-session bridge.
...
Tilecast Edge lives in `apps/edge` and is still in development; no release is published yet. Read [`apps/edge/README.md`](https://github.com/gbyo/tilecast/blob/main/apps/edge/README.md) and [`apps/edge/AGENTS.md`](https://github.com/gbyo/tilecast/blob/main/apps/edge/AGENTS.md) first. From `apps/edge`:
...
The renderer and the session bridge are C projects built with CMake on Linux. The session bridge needs GLib, json-glib, GStreamer, and WirePlumber 0.5 development packages. Hardware tests never touch a real TV or monitor: test daemons use empty device directories, and the HDMI-CEC test in CI uses the kernel's `vivid` virtual driver. The [hardware capability matrix](https://github.com/gbyo/tilecast/blob/main/docs/tilecast-edge-capabilities.md) says what each feature needs and how it is tested.
...
- [Tilecast Edge architecture](https://github.com/gbyo/tilecast/blob/main/docs/tilecast-edge.md) and its [implementation ledger](https://github.com/gbyo/tilecast/blob/main/docs/tilecast-edge-next.md) describe the Linux player in development.
