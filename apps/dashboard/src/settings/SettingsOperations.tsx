...
export function PlayerUpdatesPanel({
  ...
  return (
    <div className="grid gap-4">
      ...
      >
        <TabsList variant="line" aria-label="Player platform">
          ...
          <section className="grid gap-3 rounded-xl border border-border p-4">
            <header className="flex flex-wrap items-start justify-between gap-3">
              <div className="grid gap-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Upload a signed release directly or optionally synchronize
                  from <code>Gibsonmb71/tilecast</code>.
                </p>
              </div>
              {owner && (
              ...
              <>
                ...
                {releaseItems.length > defaultVisibleReleaseCount && (
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-sm text-muted-foreground">
                      Showing {visibleReleaseItems.length} of{" "}
                      {releaseItems.length} releases, newest first.
                    </span>
                    <Button
                      variant="ghost"
                      size="sm"
            ...
            <section className="grid gap-4 rounded-xl border border-border p-4">
              ...
              <div className="grid gap-4 border-b border-border pb-4 sm:grid-cols-2">
                ...
                <Field>
                  ...
                  />
                  <small className="text-xs text-muted-foreground">
                    Remaining targets wait until every canary reconnects. Use 0
                    to deploy to all targets at once.
                  </small>
                </Field>
                {mode === "maintenance_window" && (
                  <Field>
                    ...
                    />
                    <small className="text-xs text-muted-foreground">
                      Players install at or after this local time on each
                      screen.
                    </small>
                  </Field>
                )}
                  ...
                  <div className="grid gap-3 text-sm">
                    ...
                    {offlineTargets > 0 && (
                      <p>
                        ...
                        {offlineTargets === 1 ? "screen is" : "screens are"}{" "}
                      ...
                      <TableRow>
                        ...
                        >
                          <span className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                            ...
                            No {platformLabel} Player deployments have been
                            created.
                          </span>
                        </TableCell>
                      </TableRow>
    ...
    : ["failed", "error"].includes(value)
      ...
      : ["active", "pending", "downloading", "verified_manifest"].includes(
        ...
        : value === "paused"
          ? "warning"
          : "neutral";
  return <Badge {...statusBadgeAppearance(tone)}>{humanize(value)}</Badge>;
}
