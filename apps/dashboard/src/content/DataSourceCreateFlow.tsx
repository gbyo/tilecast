      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            >
              {t("sources.createFlow.title")}
            </h2>
            <p className="text-sm text-muted-foreground">
              {description ??
                t("sources.createFlow.intro", {
          ...
          {definitions.offered.map((definition) => (
            ...
            >
              ...
              <span className="text-xs font-normal text-muted-foreground">
                {providerGalleryDescription(definition, t)}
              </span>
            </RheaButton>
          ))}
        </div>
...
export function DataSourceCreateShell({
  ...
  const copy = resolveSetup(provider, definition, t);
  const resolvedBackLabel = backLabel ?? t("sources.createFlow.backDefault");
  const label =
    ...
    <div className="grid w-full min-w-0 gap-5">
      <div className="grid gap-4">
        <RheaButton type="button" variant="ghost" onClick={onClose}>
          <ArrowLeft size={16} aria-hidden="true" /> {resolvedBackLabel}
        </RheaButton>
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            ...
            <h2 className="text-xl font-semibold">
              {t("sources.createFlow.createTitle", { label })}
            </h2>
      ...
      <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside
          ...
          aria-label={t("sources.createFlow.setupGuidance")}
        >
          {copy.steps.length > 0 && (
            <div className="grid gap-2">
              <h3 className="text-sm font-medium">
                {t("sources.createFlow.checklist")}
              </h3>
          ...
          <p className="flex items-start gap-2 text-xs text-muted-foreground">
            ...
            {t("sources.createFlow.advancedNote")}
          </p>
        </aside>
...
export function ConnectDataFlow({
  ...
  const dialogLabel = provider
    ? t("sources.createFlow.createTitle", {
        label:
          definition && !definition.legacyEditor
            ? definition.name
            : providerLabel(provider, t),
  ...
  if (!provider)
    return createPortal(
      <DataSourceProviderGallery
        providers={providers}
        exclude={exclude}
        description={t("sources.createFlow.connectDescription")}
    ...
    >
      ...
      >
        ...
        <DataSourceCreateShell
          ...
          csrf={csrf}
          backLabel={t("sources.createFlow.allProviders")}
          onClose={onBack}
