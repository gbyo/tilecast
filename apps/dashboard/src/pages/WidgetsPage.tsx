...
export function WidgetsPage() {
  ...
  const allTypesLabel = t("widgets.list.allTypes");
  ...
  return (
    <section className="w-full min-w-0 space-y-5">
      ...
      <DashboardListToolbar>
        ...
        >
          <SelectTrigger
            aria-label={t("widgets.list.providerFilter")}
            className="w-56 max-sm:flex-1"
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="">{allTypesLabel}</SelectItem>
        ...
        <ToggleGroup
          aria-label={t("widgets.list.viewLabel")}
          variant="outline"
          spacing={0}
          multiple={false}
          value={[view]}
      ...
      {widgets.isLoading ? (
        ...
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              {t("widgets.list.emptyDescription")}
            </EmptyDescription>
          </EmptyHeader>
          {canManage && (
            <EmptyContent>
...
export function WidgetEditorPage() {
  ...
  if (id && widget.isLoading)
    return (
      <Skeleton className="h-24" aria-label={t("widgets.editor.loading")} />
    );
  if (definitions.isLoading)
    return (
      <Skeleton
        className="h-24"
        aria-label={t("widgets.editor.loadingDefinition")}
      />
    );
  if ((id && !asset) || !provider || !definition) {
    return (
      <section className="w-full min-w-0 space-y-5">
        <Empty>
          <EmptyHeader>
            <EmptyTitle>{t("widgets.editor.unavailableTitle")}</EmptyTitle>
            <EmptyDescription>
              {t("widgets.editor.unavailableDescription")}
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button type="button" onClick={close}>
              {t("widgets.editor.backToList")}
            </Button>
          </EmptyContent>
        </Empty>
      </section>
  ...
  return (
    <section className="app-editor-route">
      ...
      {asset && (
        <UsedByPanel
          emptyMessage={t("widgets.editor.noUsage")}
          groups={[
            {
              label: t("widgets.editor.playlistsLabel"),
              items: asset.playlistsUsing ?? [],
              to: (playlistId) => `/playlists/${playlistId}`,
            },
            {
              label: t("widgets.editor.layoutsLabel"),
              items: (asset.layoutUsage ?? []).map((usage) => ({
                id: usage.id,
                name: usage.name,
                hint: usage.published
                  ? t("widgets.editor.publishedHint")
                  : t("widgets.editor.draftHint"),
              })),
