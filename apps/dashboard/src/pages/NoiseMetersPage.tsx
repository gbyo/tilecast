...
import { ViewTabs } from "../components/legacy-ui";
import {
  RegisterCheckbox,
  TargetFields,
  canManage,
    ...
    <main className="grid gap-4">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="grid min-w-0 gap-1">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Noise Meter</h1>
      ...
      {instances.isError && (
        <Alert variant="destructive">
          <AlertDescription>Noise meters could not be loaded.</AlertDescription>
        </Alert>
      )}
      {remove.isError && (
        <Alert variant="destructive">
      ...
      {showEmptyState ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Create an instance to watch room noise on selected Linux players.
            </EmptyDescription>
          </EmptyHeader>
          {manageable && (
            <Link
